...
`pytest` should report **140 passed**. Two historical failures are now fixed at
the source rather than tolerated:

| Was failing | Root cause | Fix |
| --- | --- | --- |
| `test_discover_bundles_reads_real_build_directories`, `..._api_bundles_...` | a stray **empty** `build/CL-001/` (remains of an interrupted run) was reported as a bundle | `discover_bundles()` now skips `CL-*` dirs with no files, and the empty dir is gone |
| `test_browse_workspace_lists_real_project_entries` | `browse_workspace()` resolved a path *before* checking containment, so **every** symlink leaving the workspace was refused — `data/law` had been unreachable silently, with no test covering it | containment is now checked lexically, on the unresolved path (still refuses `../../`), then resolved for existence |

> **`data/transcripts/html/` is a symlink, not a vendored copy.** It points at
> `../../../../olivia/_shared/cases/la8159/02-transcripts/transcripts_rendered/I-002`
> (27 `.html` files, byte-identical at the time of linking). It was previously 27 committed
> real files, which meant **every** `transcription/` render rewrote ~5k lines in this
> repo. It is a *render*, not a source of truth — the JSON is the contract. Note the
> link crosses repos: it assumes `olivia` is checked out as a sibling of
> `mcp-ecosystem`.
