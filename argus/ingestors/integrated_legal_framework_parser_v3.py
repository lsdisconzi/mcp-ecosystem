...
class IntegratedLegalFrameworkParser:
    ...
    # Add this method after _get_framework_code:
    ...
    def build_enhanced_legal_prompt(self, config, extracted_sections, framework_text, skip_brazilian=False):
        ...
        normalized_name = self.extract_framework_name(framework_name)
        violation_id = self._generate_violation_id(framework_code, normalized_name)
        description = self._generate_description(framework_name, jurisdiction)
        
        # Create a hash for the action ID
        hash_input = f"{violation_id}|{description}"
        action_hash = hashlib.md5(hash_input.encode()).hexdigest()[:8]
    ...
    "entities": {{
        ...
        {{
            ...
            "jurisdiction": "{jurisdiction.upper() if len(jurisdiction) <= 10 else jurisdiction[:10].upper()}",
            ...
            "confidence": 0.85,
            "status": "OPEN",
            "actor_ids": ["ACT_service_provider", "ACT_<name>"],
            "action_ids": ["ACTN_<hash>"],
            "legal_article_ids": ["{framework_code}_<article_code>"],
        ...
        {{
            ...
            "end_time": "<timestamp>",
            "transcript_segment_ids": ["SEG_<hash>"],
            "checksum": "<sha256_of_deterministic_fields>"
        }}
        ],
        "segments": [
        ...
        {{
            "actor_id": "ACT_LATAM",
            "name": "LATAM Airlines",
            "normalized_name": "LATAM_AIRLINES",
    ...
    Actor ID: ACT_{normalized_name}

    Action ID: ACTN_{action_hash}
    ...
    Analysis: [Explain violation with severity integrated: "This represents a [SEVERITY] violation as [justification]..." DO NOT create separate "Severity:" lines.]
    ...
    After SECTION 2, you MUST output a COMPLETE canonical bundle in JSON format:
    ...
    "entities": {{
        ...
        {{
            "violation_id": "VIOL_<12_char_sha256>",
            "case_id": "CASE_<yyyymmdd_hhmmss>",
            "jurisdiction": "{jurisdiction.upper()[:10]}",
            "framework_code": "{framework_code}",
            "category": "<violation_category>",
            "violation_type": "<exact_violation_name_from_section_1>",
            "severity": "HIGH|MEDIUM|LOW|UNKNOWN",
            "confidence": 0.85,
            "status": "OPEN",
            "actor_ids": ["ACT_SERVICE_PROVIDER", "ACT_AFFECTED_CONSUMER"],
            "action_ids": ["ACTN_<generated_hash>"],
            "legal_article_ids": ["{framework_code}_<normalized_article_code>"],
            "evidence_ids": ["EVID_<generated_hash>"],
            "checksum": "<sha256_hash_of_deterministic_fields>"
        }}
        ],
        "evidences": [
        {{
            "evidence_id": "EVID_<generated_hash>",
            "case_id": "CASE_<yyyymmdd_hhmmss>",
            "type": "transcript",
            "source": "transcript_audio",
            "excerpt": "<exact_quote_from_transcript>",
            "start_time": "<timestamp>",
            "end_time": "<timestamp>",
            "transcript_segment_ids": ["SEG_<generated_hash>"],
            "checksum": "<sha256_hash_of_deterministic_fields>"
        }}
        ],
        "segments": [
        {{
            "segment_id": "SEG_<generated_hash>",
            "case_id": "CASE_<yyyymmdd_hhmmss>",
            "speaker": "<speaker_name>",
            "start_time": "<timestamp>",
            "end_time": "<timestamp>",
        ...
        {{
            "actor_id": "ACT_AFFECTED_CONSUMER",
            "name": "<name>",
            "normalized_name": "<name>",
            "actor_type": "individual",
            "role": "passenger",
        ...
        "actions": [
        {{
            "action_id": "ACTN_<generated_hash>",
            "description": "<action_description>",
            "sequence_index": 1,
            "actor_id": "ACT_LATAM",
            "checksum": "<sha256_hash_of_deterministic_fields>"
    ...
    For violation_id: hash(case_id|framework_code|article_ids|violation_type)[:12]

    For evidence_id: hash(case_id|"transcript"|source|start|end|excerpt)[:8]

    For segment_id: hash(case_id|speaker|start|end|text)[:8]

    For article_id: {framework_code}_<normalized_article_code>

    For actor_id: ACT_{normalized_name}

    For action_id: ACTN_{action_hash}[:8]

    Keep IDs consistent across all violations
...
"""
    ...
    def generate_canonical_schema(self, config: Dict) -> Dict:
        ...
        schema = {
            "article_id_format": "{framework_code}_<normalized_article_code>",
            "actor_id_format": "ACT_{actor_normalized_name}",
            "action_id_format": "ACTN_{hash(violation_id|violation_description)}[:8]",
        }
        return {
            "canonical_schema": {
                "version": "2.0",
...
3. **REQUIRED STRUCTURE** - Each violation MUST be:
    ...
    **ID Generation:**
    ...
    - article_id: "{self._get_framework_code(framework_name)}_<normalized_article_code>"
    ...
    **Example Canonical Output:**
    ...
    {{
        ...
        "framework_code": "{self._get_framework_code(framework_name)}",
        ...
        "legal_article_ids": ["{self._get_framework_code(framework_name)}_ART_6"],
