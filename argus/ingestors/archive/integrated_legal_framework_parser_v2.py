...
class IntegratedLegalFrameworkParser:
    ...
    def build_enhanced_legal_prompt(self, config: Dict, articles: Dict, framework_text: str, skip_brazilian: bool = False) -> str:
        ...
        UPDATED: Enhanced with strict formatting rules to prevent metadata pollution
        """
...
4. **Forbidden patterns - DO NOT generate:**
...
"""
    ...
    def _format_articles_for_prompt(self, articles: Dict) -> str:
        ...
        for article in articles.get("numbered_articles", []):
            if article["juridical_value"]["hierarchy"] in ["constitutional", "principled", "mandatory"]:
...
3.1 **VIOLATIONS CANONICAL SCHEMA**:
    - const CANONICAL_SCHEMA = ${json.dumps(self.generate_canonical_schema(config), indent=2)};
    - const CANONICAL_PROMPT_SECTION = `## CANONICAL VIOLATION OUTPUT REQUIREMENT

    After your standard analysis, you MUST ALSO output a canonical JSON array. Format:

    \`\`\`canonical
    [
    {
        "violation_id": "VIOL_<VIOLATION_ID>",
        "case_id": [],
        "jurisdiction": "${config.get('jurisdiction', 'Unknown')}",
        "framework_code": "${self._get_framework_code(config['framework_name'])}",
        "violation_type": "Exact violation name from analysis",
        "severity": [],
        "confidence": [],
        "status": "OPEN",
        "actor_ids": [],
        "action_ids": ["ACTN_a1b2c3d4"],
        "legal_article_ids": ["${self._get_framework_code(config['framework_name'])}<article>"],
        "evidence_ids": ["EVID_12345678"],
        "checksum": "sha256_hash"
    }
    ]
    ```
    Generate this JSON array for EACH discrete violation.`;
