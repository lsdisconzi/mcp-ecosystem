...
# -*- coding: utf-8 -*-
"""
Enhanced Image‑Based Report Processing Pipeline

Features:
- Image preprocessing (deskew, contrast) to improve OCR
- Persistent knowledge base (JSON) for learning patterns
- Post‑run refinement: cross‑file consistency and correction using knowledge
- Robust extraction, CSV output, and smart renaming
"""
...
def preprocess_image(image: Image.Image) -> Image.Image:
    ...
    # Use OpenCV for deskew and adaptive threshold
...
class KnowledgeBase:
    def __init__(self, filepath: str):
        ...
        self.data = {
            "patients": [],          # list of canonical patient names
            "clinics": [],           # list of canonical clinic names
            "report_types": [],      # list of observed report types
            "field_corrections": {}  # mapping from OCR variant to canonical value
        }
        self.load()
    ...
    def load(self):
        if os.path.exists(self.filepath):
            try:
                ...
                logging.info(f"Loaded knowledge base from {self.filepath}")
    ...
    def save(self):
        try:
            ...
            logging.info(f"Saved knowledge base to {self.filepath}")
        except Exception as e:
            logging.error(f"Failed to save knowledge base: {e}")

    def add_patient(self, name: str):
        if not name or name in self.data["patients"]:
            return
        # Clean name (remove trailing punctuation)
        cleaned = re.sub(r'[,;:.]$', '', name).strip()
        if cleaned:
            self.data["patients"].append(cleaned)
            self.save()

    def add_clinic(self, name: str):
        if not name or name in self.data["clinics"]:
            return
        cleaned = name.strip()
        if cleaned:
            self.data["clinics"].append(cleaned)
            self.save()

    def add_report_type(self, rtype: str):
        if not rtype or rtype in self.data["report_types"]:
            return
        cleaned = rtype.strip()
        if cleaned:
            self.data["report_types"].append(cleaned)
            self.save()

    def add_correction(self, original: str, corrected: str):
        if not original or not corrected:
            return
        self.data["field_corrections"][original] = corrected
        self.save()

    def get_canonical_patient(self, variant: str) -> Optional[str]:
        if not variant:
            return None
        if not HAS_FUZZY:
            return variant
        # Exact match
        if variant in self.data["patients"]:
            return variant
        # Fuzzy match
        matches = process.extract(variant, self.data["patients"], scorer=fuzz.ratio, limit=1)
        if matches and matches[0][1] >= 85:
            return matches[0][0]
        return None

    def get_canonical_clinic(self, variant: str) -> Optional[str]:
        if not variant:
            return None
        if not HAS_FUZZY:
            return variant
        if variant in self.data["clinics"]:
            return variant
        matches = process.extract(variant, self.data["clinics"], scorer=fuzz.ratio, limit=1)
        if matches and matches[0][1] >= 85:
            return matches[0][0]
        return None

    def correct_field(self, field: str, value: str) -> str:
        """Apply known corrections to a field value."""
        if not value:
            return value
        # Check if the exact value is in corrections
        if value in self.data["field_corrections"]:
            return self.data["field_corrections"][value]
        # Also try fuzzy matching on corrections keys
        if HAS_FUZZY:
            keys = list(self.data["field_corrections"].keys())
            matches = process.extract(value, keys, scorer=fuzz.ratio, limit=1)
            if matches and matches[0][1] >= 80:
                return self.data["field_corrections"][matches[0][0]]
        return value
...
def standardize_date(date_str: str) -> str:
    ...
    if not date_str:
        return ""
    # Remove extra text (e.g., times)
    date_str = re.sub(r'\d{2}:\d{2}:\d{2}', '', date_str).strip()
    ...
    for pat, repl in patterns:
        new = re.sub(pat, repl, date_str)
        if new != date_str:
            return new
    return date_str

# ----------------------------------------------------------------------
# File scanning & OCR (with preprocessing)
# ----------------------------------------------------------------------
...
def extract_text_from_image(image_path: str, ocr_lang: str = "por+eng", preprocess: bool = True) -> Dict[str, Any]:
    result = {
...
class LLMAnalyzer:
    def __init__(self, config: Dict[str, Any]):
        self.provider = config.get("llm_provider", "deepseek").lower()
...
# ----------------------------------------------------------------------
# Post‑run Refinement using Knowledge Base and cross‑file consistency
# ----------------------------------------------------------------------
def refine_analyses(analysis_results: Dict[str, Dict[str, Any]], kb: KnowledgeBase,
                    file_index: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """Apply knowledge base corrections and cross‑file consistency to improve results."""
    logging.info("Starting post‑run refinement...")
    corrected_count = 0

    # 1. Gather all extracted patient names, clinics, report types to learn
    for path, analysis in analysis_results.items():
        if "error" in analysis:
            continue
        meta = analysis.get("report_metadata", {})
        patient = meta.get("patient", "")
        clinic = meta.get("company_name", "")
        rtype = meta.get("report_type", "")
        if patient:
            kb.add_patient(patient)
        if clinic:
            kb.add_clinic(clinic)
        if rtype:
            kb.add_report_type(rtype)

        # Add corrections for any malformed fields we can detect
        # e.g., common OCR errors: "DESCONSI," -> "DESCONSI"
        ...
        for field, value in meta.items():
            ...
            if cleaned != value:
                kb.add_correction(value, cleaned)

    # 2. For each analysis, apply known corrections and standardise dates
    for path, analysis in analysis_results.items():
        if "error" in analysis:
            continue
        meta = analysis.get("report_metadata", {})
        # Correct patient name using KB
        patient = meta.get("patient", "")
        if patient:
            canonical = kb.get_canonical_patient(patient)
            if canonical and canonical != patient:
                meta["patient"] = canonical
                corrected_count += 1
                logging.debug(f"Corrected patient '{patient}' -> '{canonical}'")

        # Correct clinic name
        clinic = meta.get("company_name", "")
        if clinic:
            canonical = kb.get_canonical_clinic(clinic)
            if canonical and canonical != clinic:
                meta["company_name"] = canonical
                corrected_count += 1
                logging.debug(f"Corrected clinic '{clinic}' -> '{canonical}'")

        # Standardise dates
        for date_field in ["period_covered", "report_generation_date"]:
            val = meta.get(date_field, "")
            if val:
                std = standardize_date(val)
                if std != val:
                    meta[date_field] = std
                    corrected_count += 1

        # Apply field corrections (exact or fuzzy)
        # For each string field, run through correction
        for k, v in meta.items():
            if isinstance(v, str) and v:
                corrected = kb.correct_field(k, v)
                if corrected != v:
                    meta[k] = corrected
                    corrected_count += 1

        # Update analysis
        analysis["report_metadata"] = meta

    # 3. Cross‑file consistency: infer missing patient names
    # If a patient name appears in >1 file, and one file is missing, fill it
    patient_counts = {}
    for path, analysis in analysis_results.items():
        if "error" in analysis:
            continue
        patient = analysis.get("report_metadata", {}).get("patient", "")
        if patient:
            patient_counts[patient] = patient_counts.get(patient, 0) + 1

    # If one patient name is dominant (appears in >= 2 files), apply to missing ones
    if patient_counts:
        most_common = max(patient_counts, key=patient_counts.get)
        if patient_counts[most_common] >= 2:
            for path, analysis in analysis_results.items():
                if "error" in analysis:
                    continue
                meta = analysis.get("report_metadata", {})
                if not meta.get("patient", ""):
                    meta["patient"] = most_common
                    corrected_count += 1
                    logging.info(f"Inferred patient '{most_common}' for {os.path.basename(path)}")

    # Similarly for clinic name
    clinic_counts = {}
    for path, analysis in analysis_results.items():
        if "error" in analysis:
            continue
        clinic = analysis.get("report_metadata", {}).get("company_name", "")
        if clinic:
            clinic_counts[clinic] = clinic_counts.get(clinic, 0) + 1
    if clinic_counts:
        most_common_clinic = max(clinic_counts, key=clinic_counts.get)
        if clinic_counts[most_common_clinic] >= 2:
            for path, analysis in analysis_results.items():
                if "error" in analysis:
                    continue
                meta = analysis.get("report_metadata", {})
                if not meta.get("company_name", ""):
                    meta["company_name"] = most_common_clinic
                    corrected_count += 1
                    logging.info(f"Inferred clinic '{most_common_clinic}' for {os.path.basename(path)}")

    # 4. Update file_name_suggestion and directory_suggestion based on refined metadata
    for path, analysis in analysis_results.items():
        if "error" in analysis:
            continue
        meta = analysis.get("report_metadata", {})
        patient = meta.get("patient", "")
        clinic = meta.get("company_name", "")
        gen_date = meta.get("report_generation_date", "")
        rtype = meta.get("report_type", "Report")

        # Generate a clean filename if not already set or if we can improve
        if not analysis.get("file_name_suggestion") or analysis.get("file_name_suggestion") == "":
            if patient and gen_date:
                # Clean patient name: remove extra commas, spaces
                clean_patient = re.sub(r'[,;:]', '', patient).replace(' ', '_')
                filename = f"{rtype}_{gen_date}_{clean_patient}.pdf"
            elif clinic and gen_date:
                clean_clinic = re.sub(r'[^a-zA-Z0-9]', '_', clinic)
                filename = f"{rtype}_{gen_date}_{clean_clinic}.pdf"
            else:
                filename = f"{rtype}_{gen_date}.pdf" if gen_date else f"{rtype}.pdf"
            analysis["file_name_suggestion"] = filename
            corrected_count += 1

        # Directory suggestion: if patient exists, use Medical/Reports/PatientName/
        if patient and not analysis.get("directory_suggestion"):
            clean_patient = re.sub(r'[,;:]', '', patient).replace(' ', '_')
            analysis["directory_suggestion"] = f"Medical/Reports/{clean_patient}/"
            corrected_count += 1

    logging.info(f"Refinement complete. Applied {corrected_count} corrections.")
...
def run_pipeline(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kb_path = os.path.join(workspace, config.get("knowledge_base_file", "knowledge_base.json"))
    ...
    logger.info("="*80)
    logger.info("Enhanced Image‑Based Report Processing Pipeline")
    ...
    extraction_results = {}
    preprocess = config.get("preprocess_images", True)
    for i, fi in enumerate(file_index, 1):
        logger.info(f"OCR {i}/{len(file_index)}: {fi['filename']}")
        extr = extract_text_from_image(fi["full_path"], ocr_lang=config["ocr_language"], preprocess=preprocess)
    ...
    # Step 4: Refinement (NEW)
    if analysis_results:
        logger.info("Step 4: Post‑run refinement using knowledge base...")
        analysis_results = refine_analyses(analysis_results, kb, file_index)
        ...
        for path, analysis in analysis_results.items():
            ...
            with open(analysis_path, 'w', encoding='utf-8') as f:
                json.dump(analysis, f, ensure_ascii=False, indent=2)
        pipeline["steps"]["refinement"] = {"status": "completed", "corrected": True}
    else:
        pipeline["steps"]["refinement"] = {"status": "skipped"}

    # Step 5: CSV Conversion (after refinement)
    ...
    if llm_provider != "none" and analysis_results:
        logger.info("Step 6: Planning file renaming...")
        try:
            analyzer = LLMAnalyzer(config)
        ...
        logger.info("Step 6: Skipping file renaming")
...
# ----------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Enhanced Image‑Based Report Processing Pipeline")
    ...
    if args.no_preprocess:
        config["preprocess_images"] = False
