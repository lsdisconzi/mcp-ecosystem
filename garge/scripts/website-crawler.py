...
SAVE_DIR = '/home/garge/crawler_output/italianscot'  # where to save HTML and text
    ...
    async with async_playwright() as p:
        ...
        while queue and len(visited) < MAX_PAGES:
            ...
            if 'Security Checkpoint' in content or 'Code 11' in content:
                logger.warning("Security checkpoint detected. Solve it manually in the opened browser.")
                input("Press Enter after you have passed the checkpoint...")
                await page.reload()
                await asyncio.sleep(2)
                content = await page.content()
                if 'Security Checkpoint' in content:
                    logger.error("Checkpoint still present. Skipping page.")
                    continue
                # Wait again after bypass
                await wait_for_content(page, url)
            else:
                # Normal page – wait for dynamic content
                await wait_for_content(page, url)
