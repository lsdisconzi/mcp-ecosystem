"""
config/llm_providers.py — LLM provider registry and model catalog.
...
The catalog is the merge of:
  ...
  2. ``config/models-overrides.json`` (``added`` / ``overrides`` / ``removed``)
  3. Ollama models discovered at runtime (dynamic, local provider only)

Nothing here holds secrets — see ``config/llm_credentials.py`` for that.
"""
...
PROVIDERS: Dict[str, Dict[str, Any]] = {
    ...
    "ollama": {
        ...
        "icon": "fa-hard-drive",
        "kind": "ollama",
        "base_url": None,  # resolved from settings/env at request time
        "env_var": None,
        "requires_key": False,
    },
    "cerebras": {
...
def get_models(provider: str) -> List[Dict[str, Any]]:
    ...
    # Extra models declared in the overrides file.
    ...
    # De-duplicate (by id + tier) while preserving order.
    seen = set()
...
def _normalize_ollama_base(raw: str) -> str:
    ...
    ``OLLAMA_HOST`` is a *bind* address (often ``0.0.0.0:11436``), which is not
...
def resolve_base_url(provider: str, override: Optional[str] = None) -> Optional[str]:
    ...
    Ollama resolves from ``OLLAMA_BASE_URL``/``OLLAMA_HOST``/settings.
    """
    if override:
    ...
    if provider == "ollama":
        ...
        base = env_url or getattr(settings, "ollama_base_url", "http://localhost:11436")
        return _normalize_ollama_base(str(base))
