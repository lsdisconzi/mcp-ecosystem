"""
api/llm_router.py — LLM provider configuration and unified chat routing.
...
API keys are resolved server-side from ``config/llm_credentials.py``; the
...
from config import llm_credentials as creds
...
async def _ollama_models() -> List[Dict[str, Any]]:
    """Model entries for the local Ollama provider (dynamic)."""
    try:
        from api.chat import _get_ollama_models
...
def _auth_headers(provider: str, api_key: Optional[str]) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        headers.setdefault("HTTP-Referer", "http://localhost:8066")
        headers.setdefault("X-Title", "Awareness-AI Qdrant Manager")
    return headers
...
async def set_llm_provider_key(provider: str, payload: ApiKeyPayload):
    ...
    creds.set_api_key(provider, payload.api_key)
    return {
        "provider": provider,
        "configured": True,
...
async def verify_llm_provider_key(provider: str, payload: Optional[ApiKeyPayload] = None):
    ...
    if provider_kind(provider) == "ollama":
        ...
        return {"provider": provider, "ok": True, "model_count": len(models)}
...
async def llm_chat_completions(req: ChatRequest):
    ...
    if resp.status_code >= 400:
        return JSONResponse(
            ...
            content={
                "error": {"message": f"{defn['label']} API error: {resp.text[:500]}"},
