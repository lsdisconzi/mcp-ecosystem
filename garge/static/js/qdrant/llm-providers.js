/**
 * llm-providers.js — shared LLM provider / model / API-key UI for the Qdrant manager.
 *
 * Goals (see /v1/llm/* in api/llm_router.py):
 *   - The user never types an API endpoint. Base URLs and provider routing live
 *     server-side (config/llm_providers.py).
...
(function (global) {
  ...
  const STORAGE_KEY = 'qdrant.llm.selection';

  /** Order used to pick a sensible default provider (first usable wins). */
  const PROVIDER_PREFERENCE = [
    'ollama', 'deepseek', 'cerebras', 'xai', 'anthropic', 'openrouter', 'fireworks',
  ];
  ...
  /** A provider can be used for chat when its key requirement is satisfied and it has models. */
  ...
  function renderInlineKey(scope) {
    ...
    host.innerHTML = `
      <div class="apikey-row" data-provider="${esc(provider.id)}">
        ...
        <button type="button" class="btn btn-sm btn-secondary" data-action="verify">Test</button>
      </div>`;
  }
  ...
  function apiKeyRowHtml(provider) {
    ...
    const controls = provider.requires_key
      ? `<input type="password" data-role="key-input" placeholder="Paste API key…" autocomplete="off" spellcheck="false">
         <button type="button" class="btn btn-sm btn-primary" data-action="save">Save</button>
         <button type="button" class="btn btn-sm btn-secondary" data-action="verify">Test</button>
         <button type="button" class="btn btn-sm btn-danger" data-action="clear">Remove</button>`
      : `<button type="button" class="btn btn-sm btn-secondary" data-action="verify">Test connection</button>`;
  ...
  async function renderApiKeysModal(providers) {
    ...
    if (!providers) {
      ...
      host.innerHTML = '<p style="color:var(--gray);font-size:13px;">Loading providers…</p>';
      try {
        providers = await loadProviders(true);
      } catch (err) {
        host.innerHTML = `<p style="color:var(--red);font-size:13px;">${esc(err.message)}</p>`;
        return;
      }
    }
  ...
  function openApiKeysModal(scopeName) {
    ...
    // `.modal` uses flex for centering, so don't override it with `block`.
  ...
  async function saveKey(providerId, rawKey, row, button) {
    ...
    try {
      ...
      notify(`API key saved for ${providerId}.`, 'success');
      if (row) {
  ...
  async function clearKey(providerId) {
    ...
    try {
      ...
      notify(`API key removed for ${providerId}.`, 'info');
      await refresh(true);
      syncOpenApiKeysModal();
    } catch (err) {
  ...
  async function verifyKey(providerId, rawKey, row, button) {
    ...
    try {
      ...
      if (data.ok) {
        ...
        notify(`${providerId}: connection OK${suffix}.`, 'success');
      } else {
